  ...
  <div class="x_panel">
    <div class="x_title">
      ...
      <ul class="nav navbar-right panel_toolbox">
        ...
        <li><a class="close-link"><i class="fa fa-close"/></a>
        </li>
      </ul>
      <div class="clearfix"/>
    </div>
    <div class="x_content">
      <table class="table table-bordered elog-journal-table">
        <thead>
          ...
          <tr>
            <template v-for="denser, key in headers.densers">
              <th class="rotate densers-header">
                <div><span>pH</span></div>
              </th>
              <th class="rotate">
                <div><span>Медь, мг/л</span></div>
              </th>
              <th class="rotate">
                <div><span>Железо, мг/л</span></div>
              </th>
              <th class="rotate">
                <div><span>Ж:Т</span></div>
              </th>
        ...
        <tbody>
          <template v-if="items">
            <tr 
              v-for="(row, combine_id) in items" 
              @click="editRow(combine_id)">
              ...
              <template v-for="denser in row.item">
                <template v-if="editableId === combine_id">
                  <td><input 
                    type="text" 
                    class="form-control" 
                    v-model="denser.hs.ph"></td>
                  <td><input 
                    type="text" 
                    class="form-control" 
                    v-model="denser.hs.cu"></td>
                  <td><input 
                    type="text" 
                    class="form-control" 
                    v-model="denser.hs.fe"></td>
                  <td><input 
                    type="text" 
                    class="form-control" 
                    v-model="denser.hs.liq_sol"></td>

                  <td><input 
                    type="text" 
                    class="form-control" 
                    v-model="denser.ls.ph"></td>
                  <td><input 
                    type="text" 
                    class="form-control" 
                    v-model="denser.ls.liq_sol"></td>
                </template>
                <template v-else>
                  <td>{{ denser.hs.ph }}</td>
                  <td>{{ denser.hs.cu }}</td>
          ...
          <tr>
            <td 
              ...
              style="width:80px">
              <select-hours 
                :current="paramSelectedHour" 
                @selectedHour="onSelectedHour"/>
            </td>
            <td>
              <a 
            ...
            <td colspan="20">
              <template v-if=" editableId != ''">
                <a 
                  ...
                  align="left" 
                  @click="saveRecord()">
                  <i class="glyphicon glyphicon-floppy-disk"/>Сохранить
                </a>
              </template>
              <template v-else>
                <a 
                  ...
                  align="left" 
                  @click="addRecord()">
                  <i class="glyphicon glyphicon-floppy-disk"/>Добавить
                </a>
              </template>
            </td>
          </tr>
