<template>
    <v-popover offset="16" :disabled="!(mode==='validate')">
      <input :class="classes"
       :name="fieldName"
       :row-index="rowIndex"
       :value="value"
      ...
      />
      <i v-if="$store.getters.cellComment(tableName, fieldName, rowIndex)" class="far fa-envelope comment-notification"></i>
      <template slot="popover">
        <CellComment :table-name="tableName" :field-name="fieldName" :row-index="rowIndex"></CellComment>
      </template>
    </v-popover>
</template>
...
export default {
  ...
  data() {
    return {
        classes: 'general-value number-cell form-control tooltip-target',
        minValue: null,
        maxValue: null,
        type: null,
        placeholder: '',
  ...
  computed: {
    hasComment: function () {
      this.$store.getters.cellComment(this.tableName, this.fieldName, this.rowIndex) ? true : false;
    },
    tableName: function () {
