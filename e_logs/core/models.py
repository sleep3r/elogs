...
from e_logs.common.all_journals_app.models import Field, Journal, Plant, Table
...
class Setting(StrAsDictMixin, models.Model, metaclass=SettingsMeta):
    ...
    class Meta:
        # equivalent to unique_together = [name, employee, scope], but this
        # works
    ...
    scopes_attrs = ((Field, 'field'), (Table, 'table'), (Journal, 'journal'), (Plant, 'plant'))
    ...
    def get_value(name: str, obj=None, employee: Optional[Employee] = None) -> Any:
        ...
        qs = Setting.objects.filter(**{'name': name, 'employee': employee})
            ...
            for (scope, attr) in Setting.scopes_attrs:
                ...
                if isinstance(obj, scope):
                    found_setting = Setting.objects.filter(
                        **{'name': name, 'employee': employee, attr: obj}
                    ).first()
                if found_setting:
                ...
                except BaseException:  # if haven't found, we'll search father
                    pass
