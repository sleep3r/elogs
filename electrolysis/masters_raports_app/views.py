...
@login_required
def index(request):
    context = get_common_context(journal_name="masters_report", request=request)
    context.journal_title = 'Журнал рапортов мастеров смен'

    template = loader.get_template('common.html')

    seria1_table = deep_dict()
    seria1_table.title = "1-я 2-я серия"
